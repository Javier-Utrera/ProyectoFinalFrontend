...
export class EditarRelatoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      yo: this.apiService.getUsuarioPorToken()
    }).subscribe({
      next: ({ relato, yo }) => {
        console.log(yo);
        this.relato = relato;
        // --- detectar creador original según participaciones ---
        const partArray = Array.isArray(relato.participaciones)
          ? (relato.participaciones as Array<{ usuario: number; orden: number }>)
          : [];
        // buscamos la participación con orden===1
        const creadorId = partArray.find(p => p.orden === 1)?.usuario;
        this.esCreador = creadorId === yo.id;

        // Si eres creador, montamos el formulario:
        if (this.esCreador) {
